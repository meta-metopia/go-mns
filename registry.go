...
func RegistryContractAddress(backend bind.ContractBackend) (common.Address, error) {
	...
	currentDir = filepath.Join(currentDir, string(os.PathSeparator), "resource", string(os.PathSeparator), FileName)
	file, err := yaml.ReadFile(currentDir)
	if err != nil {
		panic(err.Error())
	}

	contractAddress, error := file.Get("mdns.contractAddress")
	if error != nil {
		panic(error.Error())
	}
